...
public class ApppointmentPast extends BaseExpandableListAdapter {
    private Context _context;
    private HashMap<String, ArrayList<Past>> _listDataChild;
    ...
    public ApppointmentPast(Context context, ArrayList<String> listDataHeader, HashMap<String, ArrayList<Past>> listChildData, MyAppointmentsPresenter myAppointmentsPresenter) {
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        final Past pastObject = (Past) ((ArrayList) this._listDataChild.get(this._listDataHeader.get(groupPosition))).get(childPosition);
        if (convertView == null) {
        ...
        try {
            tv_status.setText(MyAppointments.getStatusResponse("" + pastObject.getStatus(), pastObject.getCancellationUserType()));
        } catch (Exception e) {
            tv_status.setText(MyAppointments.getStatusResponse("" + pastObject.getStatus(), 1));
        }
        tv_name.setText(pastObject.getPatientId().getFirstName() + " " + pastObject.getPatientId().getLastName());
        tv_age.setText(pastObject.getPatientId().getGender() + "- " + pastObject.getAge());
        tv_time.setText((CharSequence) this.toTimeArr12HrList.get(this.toTimeArrList.indexOf(pastObject.getFrom())));
        ...
        Picasso.with(this._context).load(Constants.HTTP + pastObject.getProfile()).transform(new CircleTransform()).placeholder((int) C1020R.drawable.profile_circle).error((int) C1020R.drawable.profile_circle).into(img_user);
        if (isLastChild) {
        ...
        lin_parent.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (SharedPreff.getStaffLoginResponse() == null) {
                    PatientPendingData patientPendingData = new PatientPendingData();
                    patientPendingData.setAddress(pastObject.getPatientId().getAddress());
                    patientPendingData.setDOB(pastObject.getAge());
                    patientPendingData.setEmail(pastObject.getPatientId().getEmail());
                    patientPendingData.setMobile(pastObject.getPatientId().getMobile());
                    patientPendingData.setCode(pastObject.getPatientId().getCode());
                    patientPendingData.setProfilePic(pastObject.getPatientId().getProfilePic());
                    patientPendingData.setName(pastObject.getPatientId().getFirstName() + " " + pastObject.getPatientId().getLastName());
                    patientPendingData.setSubscription("");
                    patientPendingData.setGender(pastObject.getPatientId().getGender());
                    patientPendingData.setProfilePic(pastObject.getProfile());
                    patientPendingData.setBookingId(pastObject.getBookingId());
                    patientPendingData.setDrschedulesetsId(pastObject.getDrschedulesetsId());
                    patientPendingData.setPateientID(pastObject.getPatientId().get_id());
                    patientPendingData.setPatient_id(pastObject.getPatientId().get_id());
                    patientPendingData.setStatus(pastObject.getStatus());
                    ApppointmentPast.this.myAppointmentsPresenter.gotoDetailScreen(patientPendingData);
                }
            }
        });
        return convertView;
