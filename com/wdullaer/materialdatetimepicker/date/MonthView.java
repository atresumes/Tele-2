...
public abstract class MonthView extends View {
    ...
    protected class MonthViewTouchHelper extends ExploreByTouchHelper {
        ...
        protected CharSequence getItemDescription(int day) {
            ...
            return MonthView.this.getContext().getString(C1395R.string.mdtp_item_is_selected, new Object[]{date});
        }
    }
    ...
    public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
        ...
        this.mDayOfWeekTypeface = res.getString(C1395R.string.mdtp_day_of_week_label_typeface);
        this.mMonthTitleTypeface = res.getString(C1395R.string.mdtp_sans_serif);
        if (this.mController != null && this.mController.isThemeDark()) {
            darkTheme = true;
        }
        if (darkTheme) {
            this.mDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_text_normal_dark_theme);
            this.mMonthDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_month_day_dark_theme);
            this.mDisabledDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_text_disabled_dark_theme);
            this.mHighlightedDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_text_highlighted_dark_theme);
        } else {
            this.mDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_text_normal);
            this.mMonthDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_month_day);
            this.mDisabledDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_text_disabled);
            this.mHighlightedDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_date_picker_text_highlighted);
        }
        this.mSelectedDayTextColor = ContextCompat.getColor(context, C1395R.color.mdtp_white);
        this.mTodayNumberColor = this.mController.getAccentColor();
        this.mMonthTitleColor = ContextCompat.getColor(context, C1395R.color.mdtp_white);
        this.mStringBuilder = new StringBuilder(50);
        MINI_DAY_NUMBER_TEXT_SIZE = res.getDimensionPixelSize(C1395R.dimen.mdtp_day_number_size);
        MONTH_LABEL_TEXT_SIZE = res.getDimensionPixelSize(C1395R.dimen.mdtp_month_label_size);
        MONTH_DAY_LABEL_TEXT_SIZE = res.getDimensionPixelSize(C1395R.dimen.mdtp_month_day_label_text_size);
        MONTH_HEADER_SIZE = res.getDimensionPixelOffset(C1395R.dimen.mdtp_month_list_item_header_height);
        DAY_SELECTED_CIRCLE_SIZE = res.getDimensionPixelSize(C1395R.dimen.mdtp_day_number_select_circle_radius);
        this.mRowHeight = (res.getDimensionPixelOffset(C1395R.dimen.mdtp_date_picker_view_animator_height) - getMonthHeaderSize()) / 6;
    ...
    private String getMonthAndYearString() {
        ...
        if (VERSION.SDK_INT < 18) {
            pattern = getContext().getResources().getString(C1395R.string.mdtp_date_v1_monthyear);
        } else {
