...
public final class ShareApi {
    ...
    static class C05797 implements ValueMapper {
        C05797() {
        }
        ...
        stageOpenGraphAction(parameters, new OnMapperCompleteListener() {
            public void onComplete() {
                try {
                    ...
                    new GraphRequest(AccessToken.getCurrentAccessToken(), "/me/" + URLEncoder.encode(action.getActionType(), HttpRequest.CHARSET_UTF8), parameters, HttpMethod.POST, requestCallback).executeAsync();
    ...
    private static <T> void stageCollectionValues(Collection<T> collection, OnMapperCompleteListener onCollectionValuesStagedListener) {
        CollectionMapper.iterate(collection, new C05797(), onCollectionValuesStagedListener);
    }
    ...
    private static void stageOpenGraphObject(final ShareOpenGraphObject object, final OnMapValueCompleteListener onOpenGraphObjectStagedListener) {
        ...
        stageCollectionValues(collection, new OnMapperCompleteListener() {
            public void onComplete() {
                ...
                try {
                    new GraphRequest(AccessToken.getCurrentAccessToken(), "/me/objects/" + URLEncoder.encode(ogType, HttpRequest.CHARSET_UTF8), parameters, HttpMethod.POST, requestCallback).executeAsync();
                } catch (UnsupportedEncodingException ex) {
                    String message = ex.getLocalizedMessage();
                    if (message == null) {
                        message = "Error staging Open Graph object.";
