...
class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest requestData) {
        ...
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Sending report to: " + getUrl());
        int statusCode = httpRequest.code();
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Create report request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Result was: " + statusCode);
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest request, Report report) {
        ...
        if (report.getFiles().length == 1) {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Adding single file " + report.getFileName() + " to report " + report.getIdentifier());
            return request.part(FILE_PARAM, report.getFileName(), FILE_CONTENT_TYPE, report.getFile());
        }
        int i = 0;
        for (File file : report.getFiles()) {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Adding file " + file.getName() + " to report " + report.getIdentifier());
