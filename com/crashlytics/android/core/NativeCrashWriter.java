...
class NativeCrashWriter {
    ...
    private static EventMessage createEventMessage(SessionEventData crashEvent, LogFileManager logFileManager, Map<String, String> attributes) throws IOException {
        ...
        if (logByteString == null) {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "No log data to include with this event.");
        }
    ...
    private static ProtobufMessage createDeviceMessage(DeviceData deviceData) {
        ...
        return new DeviceMessage(((float) deviceData.batteryCapacity) / TrackerSettings.DEFAULT_MIN_METERS_BETWEEN_UPDATES, deviceData.batteryVelocity, deviceData.proximity, deviceData.orientation, deviceData.totalPhysicalMemory - deviceData.availablePhysicalMemory, deviceData.totalInternalStorage - deviceData.availableInternalStorage);
    }
