...
class CrashlyticsController {
    ...
    static final FilenameFilter ANY_SESSION_FILENAME_FILTER = new C04454();
    ...
    static final Comparator<File> LARGEST_FILE_NAME_FIRST = new C04432();
    ...
    static final FilenameFilter SESSION_FILE_FILTER = new C04421();
    ...
    static final Comparator<File> SMALLEST_FILE_NAME_FIRST = new C04443();
    ...
    static class C04432 implements Comparator<File> {
        C04432() {
        }
    ...
    static class C04443 implements Comparator<File> {
        C04443() {
        }
    ...
    static class C04454 implements FilenameFilter {
        C04454() {
        }
    ...
    class C04465 implements CrashListener {
        C04465() {
        }
    ...
    private static final class PrivacyDialogCheck implements SendCheck {
        ...
        class C04511 implements AlwaysSendCallback {
            C04511() {
            }
        ...
        public boolean canSendReports() {
            ...
            final CrashPromptDialog dialog = CrashPromptDialog.create(activity, this.promptData, new C04511());
            activity.runOnUiThread(new Runnable() {
                public void run() {
                    dialog.show();
                }
            });
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Waiting for user opt-in.");
            dialog.await();
    ...
    private static final class SendReportRunnable implements Runnable {
        ...
        public void run() {
            if (CommonUtils.canTryConnection(this.context)) {
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Attempting to send crash report at time of crash...");
    ...
    void enableExceptionHandling(UncaughtExceptionHandler defaultHandler) {
        openSession();
        this.crashHandler = new CrashlyticsUncaughtExceptionHandler(new C04465(), defaultHandler);
        Thread.setDefaultUncaughtExceptionHandler(this.crashHandler);
    }

    synchronized void handleUncaughtException(final Thread thread, final Throwable ex) {
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Crashlytics is handling uncaught exception \"" + ex + "\" from thread " + thread.getName());
    ...
    void submitAllReports(float delay, SettingsData settingsData) {
        if (settingsData == null) {
            Fabric.getLogger().mo3823w(CrashlyticsCore.TAG, "Could not send reports. Settings are not available.");
            return;
        }
    ...
    boolean finalizeSessions(final SessionSettingsData sessionSettingsData) {
        return ((Boolean) this.backgroundWorker.submitAndWait(new Callable<Boolean>() {
            public Boolean call() throws Exception {
                if (CrashlyticsController.this.isHandlingException()) {
                    Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Skipping session finalization because a crash has already occurred.");
                    return Boolean.FALSE;
                }
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Finalizing previously open sessions.");
                CrashlyticsController.this.doCloseSessions(sessionSettingsData, true);
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Closed all previously open sessions");
                return Boolean.TRUE;
            }
        })).booleanValue();
    }
    ...
    private void doOpenSession() throws Exception {
        ...
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Opening a new session with ID " + sessionIdentifier);
    ...
    private void doCloseSessions(SessionSettingsData sessionSettingsData, boolean excludeCurrent) throws Exception {
        ...
        if (sessionBeginFiles.length <= offset) {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "No open sessions to be closed.");
            return;
        }
        writeSessionUser(getSessionIdFromSessionFile(sessionBeginFiles[offset]));
        if (sessionSettingsData == null) {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Unable to close session. Settings are not loaded.");
        } else {
    ...
    private void closeOpenSessions(File[] sessionBeginFiles, int beginIndex, int maxLoggedExceptionsCount) {
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Closing open sessions.");
        for (int i = beginIndex; i < sessionBeginFiles.length; i++) {
            ...
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Closing session: " + sessionIdentifier);
    ...
    private void closeWithoutRenamingOrLog(ClsFileOutputStream fos) {
        if (fos != null) {
            try {
                ...
                Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Error closing session file stream in the presence of an exception", ex);
            }
        }
    }
        ...
        while (i < length) {
            ...
            if (matcher.matches()) {
                if (!sessionIdsToKeep.contains(matcher.group(1))) {
                    Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Trimming session file: " + fileName);
                    sessionPartFile.delete();
                }
                i++;
            } else {
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Deleting unknown file: " + fileName);
                sessionPartFile.delete();
                return;
            }
        }
    ...
    private File[] getTrimmedNonFatalFiles(String sessionId, File[] nonFatalFiles, int maxLoggedExceptionsCount) {
        ...
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, String.format(Locale.US, "Trimming down to %d logged exceptions.", new Object[]{Integer.valueOf(maxLoggedExceptionsCount)}));
    ...
    void doCleanInvalidTempFiles(File[] invalidFiles) {
        ...
        for (File invalidFile : invalidFiles) {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Found invalid session part file: " + invalidFile);
        ...
        if (!invalidSessionIds.isEmpty()) {
            ...
            while (i < length) {
                ...
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Moving session file: " + sessionFile);
                if (!sessionFile.renameTo(new File(invalidFilesDir, sessionFile.getName()))) {
                    Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Could not move session file. Deleting " + sessionFile);
                    sessionFile.delete();
                }
                i++;
            }
    ...
    private void writeFatal(Date time, Thread thread, Throwable ex) {
        ...
        try {
            ...
            if (currentSessionId == null) {
                Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Tried to write a fatal exception while no session was open.", null);
            ...
            try {
                ...
                outputStream = fos2;
                try {
                    Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "An error occurred in the fatal exception logger", e);
                    CommonUtils.flushOrLog(cos, "Failed to flush to session begin file.");
    ...
    private void doWriteExternalCrashEvent(SessionEventData crashEventData) throws IOException {
        ...
        try {
            ...
            if (previousSessionId == null) {
                Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Tried to write a native crash while no session was open.", null);
                CommonUtils.flushOrLog(null, "Failed to flush to session begin file.");
                CommonUtils.closeOrLog(null, "Failed to close fatal exception file output stream.");
                return;
            }
            ...
            try {
                ...
                outputStream = fos2;
                try {
                    Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "An error occurred in the native crash logger", e);
                    CommonUtils.flushOrLog(cos, "Failed to flush to session begin file.");
                    CommonUtils.closeOrLog(fos, "Failed to close fatal exception file output stream.");
                } catch (Throwable th2) {
                    th = th2;
    ...
    private void doWriteNonFatal(Date time, Thread thread, Throwable ex) {
        ...
        String currentSessionId = getCurrentSessionId();
        if (currentSessionId == null) {
            Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Tried to write a non-fatal exception while no session was open.", null);
            return;
        }
        ...
        CodedOutputStream cos = null;
        try {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Crashlytics is logging non-fatal exception \"" + ex + "\" from thread " + thread.getName());
            ...
            try {
                ...
                outputStream = fos2;
                try {
                    Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "An error occurred in the non-fatal exception logger", e);
                    CommonUtils.flushOrLog(cos, "Failed to flush to non-fatal file.");
        ...
        try {
            ...
            Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "An error occurred when trimming non-fatal files.", e4);
        }
    }
    ...
    private void writeSessionPartsToSessionFile(File sessionBeginFile, String sessionId, int maxLoggedExceptionsCount) {
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Collecting session parts for ID " + sessionId);
        ...
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, String.format(Locale.US, "Session %s has fatal exception: %s", new Object[]{sessionId, Boolean.valueOf(hasFatal)}));
        ...
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, String.format(Locale.US, "Session %s has non-fatal exceptions: %s", new Object[]{sessionId, Boolean.valueOf(hasNonFatal)}));
        if (hasFatal || hasNonFatal) {
            ...
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "No events present for session ID " + sessionId);
        }
        Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Removing session part files for ID " + sessionId);
    ...
    private void synthesizeSessionFile(File sessionBeginFile, String sessionId, File[] nonFatalFiles, File fatalFile) {
        ...
        try {
            ...
            try {
                ...
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Collecting SessionStart data for session ID " + sessionId);
                ...
                outputStream = fos2;
                try {
                    Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Failed to write session file for session ID: " + sessionId, e);
    ...
    private static void writeNonFatalEventsTo(CodedOutputStream cos, File[] nonFatalFiles, String sessionId) {
        ...
        for (File nonFatalFile : nonFatalFiles) {
            try {
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, String.format(Locale.US, "Found Non Fatal for session ID %s in %s ", new Object[]{sessionId, nonFatalFile.getName()}));
                writeToCosFromFile(cos, nonFatalFile);
            } catch (Exception e) {
                Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Error writting non-fatal to session.", e);
            }
        }
    }
    ...
    private void writeInitialPartsTo(CodedOutputStream cos, String sessionId) throws IOException {
        for (String tag : INITIAL_SESSION_PART_TAGS) {
            ...
            if (sessionPartFiles.length == 0) {
                Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Can't find " + tag + " data for session ID " + sessionId, null);
            } else {
                Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Collecting " + tag + " data for session ID " + sessionId);
    ...
    private static void writeToCosFromFile(CodedOutputStream cos, File file) throws IOException {
        ...
        Fabric.getLogger().mo3813e(CrashlyticsCore.TAG, "Tried to include a file that doesn't exist: " + file.getName(), null);
    }
    ...
    private void sendSessionReports(SettingsData settingsData) {
        if (settingsData == null) {
            Fabric.getLogger().mo3823w(CrashlyticsCore.TAG, "Cannot send reports. Settings are unavailable.");
            return;
        }
        Context context = this.crashlyticsCore.getContext();
    ...
    private static void recordLoggedExceptionAnswersEvent(String sessionId, String exceptionName) {
        ...
        if (answers == null) {
            Fabric.getLogger().mo3810d(CrashlyticsCore.TAG, "Answers is not available");
        } else {
