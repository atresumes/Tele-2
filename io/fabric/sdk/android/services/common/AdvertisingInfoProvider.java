...
class AdvertisingInfoProvider {
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        ...
        if (isInfoValid(infoToReturn)) {
            Fabric.getLogger().mo3810d(Fabric.TAG, "Using AdvertisingInfo from Preference Store");
        ...
        new Thread(new BackgroundPriorityRunnable() {
            public void onRun() {
                ...
                if (!advertisingInfo.equals(infoToStore)) {
                    Fabric.getLogger().mo3810d(Fabric.TAG, "Asychronously getting Advertising Info and storing it to preferences");
    ...
    private AdvertisingInfo getAdvertisingInfoFromStrategies() {
        ...
        if (isInfoValid(infoToReturn)) {
            Fabric.getLogger().mo3810d(Fabric.TAG, "Using AdvertisingInfo from Reflection Provider");
        } else {
            infoToReturn = getServiceStrategy().getAdvertisingInfo();
            if (isInfoValid(infoToReturn)) {
                Fabric.getLogger().mo3810d(Fabric.TAG, "Using AdvertisingInfo from Service Provider");
            } else {
                Fabric.getLogger().mo3810d(Fabric.TAG, "AdvertisingInfo not present");
            }
        }
        return infoToReturn;
    }
