...
*/
        ...
        CreateRealmRunnable(RealmNotifier notifier, RealmConfiguration configuration, InstanceCallback<T> callback, Class<T> realmClass) {
    ...
    static <T extends BaseRealm> RealmAsyncTask createRealmOrGetFromCacheAsync(RealmConfiguration configuration, InstanceCallback<T> callback, Class<T> realmClass) {
        return getCache(configuration.getPath(), true).doCreateRealmOrGetFromCacheAsync(configuration, callback, realmClass);
    }

    private synchronized <T extends BaseRealm> RealmAsyncTask doCreateRealmOrGetFromCacheAsync(RealmConfiguration configuration, InstanceCallback<T> callback, Class<T> realmClass) {
        Future<?> future;
    ...
    private synchronized <E extends BaseRealm> E doCreateRealmOrGetFromCache(RealmConfiguration configuration, Class<E> realmClass) {
        ...
        if (refAndCount.localRealm.get() == null) {
            ...
            if (realmClass == Realm.class && refAndCount.globalCount == 0) {
                storeColumnIndices(this.typedColumnIndicesArray, realm.schema.getImmutableColumnIndicies());
            }
    ...
    synchronized void updateSchemaCache(Realm realm) {
        if (((RefAndCount) this.refAndCountMap.get(RealmCacheType.TYPED_REALM)).localRealm.get() != null) {
            ColumnIndices[] globalCacheArray = this.typedColumnIndicesArray;
            ColumnIndices createdCacheEntry = realm.updateSchemaCache(globalCacheArray);
            if (createdCacheEntry != null) {
                storeColumnIndices(globalCacheArray, createdCacheEntry);
            }
        }
    }
