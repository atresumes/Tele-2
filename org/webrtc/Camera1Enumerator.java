...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    public String[] getDeviceNames() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (name != null) {
                namesList.add(name);
                Logging.m172d(TAG, "Index: " + i + ". " + name);
            } else {
                Logging.m173e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
    ...
    private static CameraInfo getCameraInfo(int index) {
        ...
        try {
            ...
            Logging.m174e(TAG, "getCameraInfo failed on index " + index, e);
            return null;
        }
    }
    ...
    private static List<CaptureFormat> enumerateFormats(int cameraId) {
        ...
        Logging.m172d(TAG, "Get supported formats for camera index " + cameraId + ".");
        long startTimeMs = SystemClock.elapsedRealtime();
        Camera camera = null;
        try {
            Logging.m172d(TAG, "Opening camera with index " + cameraId);
            ...
            try {
                ...
                Logging.m174e(TAG, "getSupportedFormats() failed on camera index " + cameraId, e);
            }
            Logging.m172d(TAG, "Get supported formats for camera index " + cameraId + " done." + " Time spent: " + (SystemClock.elapsedRealtime() - startTimeMs) + " ms.");
        } catch (RuntimeException e2) {
            Logging.m174e(TAG, "Open camera failed on camera index " + cameraId, e2);
            formatList = new ArrayList();
            if (camera != null) {
                camera.release();
            }
    ...
    static int getCameraIndex(String deviceName) {
        Logging.m172d(TAG, "getCameraIndex: " + deviceName);
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
