...
public class NetworkMonitorAutoDetect extends BroadcastReceiver {
    ...
    static class ConnectivityManagerDelegate {
        ...
        private NetworkInformation networkToInfo(Network network) {
            ...
            if (linkProperties == null) {
                Logging.m176w(NetworkMonitorAutoDetect.TAG, "Detected unknown network: " + network.toString());
                return null;
            } else if (linkProperties.getInterfaceName() == null) {
                Logging.m176w(NetworkMonitorAutoDetect.TAG, "Null interface name for network " + network.toString());
                return null;
            } else {
                NetworkState networkState = getNetworkState(network);
                ConnectionType connectionType = NetworkMonitorAutoDetect.getConnectionType(networkState);
                if (connectionType == ConnectionType.CONNECTION_NONE) {
                    Logging.m172d(NetworkMonitorAutoDetect.TAG, "Network " + network.toString() + " is disconnected");
                    return null;
                }
                if (connectionType == ConnectionType.CONNECTION_UNKNOWN || connectionType == ConnectionType.CONNECTION_UNKNOWN_CELLULAR) {
                    Logging.m172d(NetworkMonitorAutoDetect.TAG, "Network " + network.toString() + " connection type is " + connectionType + " because it has type " + networkState.getNetworkType() + " and subtype " + networkState.getNetworkSubType());
                }
        ...
        public void releaseCallback(NetworkCallback networkCallback) {
            if (supportNetworkCallback()) {
                Logging.m172d(NetworkMonitorAutoDetect.TAG, "Unregister network callback");
    ...
    private class SimpleNetworkCallback extends NetworkCallback {
        ...
        public void onAvailable(Network network) {
            Logging.m172d(NetworkMonitorAutoDetect.TAG, "Network becomes available: " + network.toString());
            onNetworkChanged(network);
        }

        public void onCapabilitiesChanged(Network network, NetworkCapabilities networkCapabilities) {
            Logging.m172d(NetworkMonitorAutoDetect.TAG, "capabilities changed: " + networkCapabilities.toString());
            onNetworkChanged(network);
        }

        public void onLinkPropertiesChanged(Network network, LinkProperties linkProperties) {
            Logging.m172d(NetworkMonitorAutoDetect.TAG, "link properties changed: " + linkProperties.toString());
            onNetworkChanged(network);
        }

        public void onLosing(Network network, int maxMsToLive) {
            Logging.m172d(NetworkMonitorAutoDetect.TAG, "Network " + network.toString() + " is about to lose in " + maxMsToLive + "ms");
        }
    ...
    public NetworkMonitorAutoDetect(Observer observer, Context context) {
        ...
        if (this.connectivityManagerDelegate.supportNetworkCallback()) {
            ...
            try {
                ...
                Logging.m176w(TAG, "Unable to obtain permission to request a cellular network.");
    ...
    private void connectionTypeChanged(NetworkState networkState) {
        ...
        if (newConnectionType != this.connectionType || !newWifiSSID.equals(this.wifiSSID)) {
            ...
            Logging.m172d(TAG, "Network connectivity changed, type is: " + this.connectionType);
