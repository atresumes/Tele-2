...
class WebRtcAudioEffects {
    ...
    public static boolean isAcousticEchoCancelerBlacklisted() {
        ...
        if (isBlacklisted) {
            Logging.m176w(TAG, Build.MODEL + " is blacklisted for HW AEC usage!");
        }
    ...
    public static boolean isNoiseSuppressorBlacklisted() {
        ...
        if (isBlacklisted) {
            Logging.m176w(TAG, Build.MODEL + " is blacklisted for HW NS usage!");
        }
        return isBlacklisted;
    }
    ...
    public static boolean canUseAcousticEchoCanceler() {
        ...
        Logging.m172d(TAG, "canUseAcousticEchoCanceler: " + canUseAcousticEchoCanceler);
    ...
    public static boolean canUseNoiseSuppressor() {
        ...
        Logging.m172d(TAG, "canUseNoiseSuppressor: " + canUseNoiseSuppressor);
    ...
    static WebRtcAudioEffects create() {
        ...
        Logging.m176w(TAG, "API level 16 or higher is required!");
        return null;
    }
    ...
    public boolean setAEC(boolean enable) {
        Logging.m172d(TAG, "setAEC(" + enable + ")");
        if (!canUseAcousticEchoCanceler()) {
            Logging.m176w(TAG, "Platform AEC is not supported");
            ...
            Logging.m173e(TAG, "Platform AEC state can't be modified while recording");
            return false;
        }
    }

    public boolean setNS(boolean enable) {
        Logging.m172d(TAG, "setNS(" + enable + ")");
        if (!canUseNoiseSuppressor()) {
            Logging.m176w(TAG, "Platform NS is not supported");
            ...
            Logging.m173e(TAG, "Platform NS state can't be modified while recording");
            return false;
        }
    }
    ...
    public void enable(int audioSession) {
        boolean z;
        boolean enable;
        Logging.m172d(TAG, "enable(audioSession=" + audioSession + ")");
        if (this.aec == null) {
            z = true;
        } else {
            z = false;
        ...
        for (Descriptor d : AudioEffect.queryEffects()) {
            if (effectTypeIsVoIP(d.type)) {
                Logging.m172d(TAG, "name: " + d.name + ", " + "mode: " + d.connectMode + ", " + "implementor: " + d.implementor + ", " + "UUID: " + d.uuid);
            }
        }
        if (isAcousticEchoCancelerSupported()) {
            ...
            if (this.aec != null) {
                ...
                if (this.aec.setEnabled(enable) != 0) {
                    Logging.m173e(TAG, "Failed to set the AcousticEchoCanceler state");
                }
                Logging.m172d(TAG, "AcousticEchoCanceler: was " + (enabled ? "enabled" : "disabled") + ", enable: " + enable + ", is now: " + (this.aec.getEnabled() ? "enabled" : "disabled"));
            } else {
                Logging.m173e(TAG, "Failed to create the AcousticEchoCanceler instance");
            }
        }
        if (isNoiseSuppressorSupported()) {
            ...
            if (this.ns != null) {
                ...
                if (this.ns.setEnabled(enable) != 0) {
                    Logging.m173e(TAG, "Failed to set the NoiseSuppressor state");
                }
                Logging.m172d(TAG, "NoiseSuppressor: was " + (enabled ? "enabled" : "disabled") + ", enable: " + enable + ", is now: " + (this.ns.getEnabled() ? "enabled" : "disabled"));
                return;
            }
            Logging.m173e(TAG, "Failed to create the NoiseSuppressor instance");
        }
    }

    public void release() {
        Logging.m172d(TAG, "release");
        if (this.aec != null) {
            this.aec.release();
            this.aec = null;
        }
