...
public class MediaCodecVideoDecoder {
    ...
    private static class TextureListener implements OnTextureFrameAvailableListener {
        ...
        public void addBufferToRender(DecodedOutputBuffer buffer) {
            if (this.bufferToRender != null) {
                Logging.m173e(MediaCodecVideoDecoder.TAG, "Unexpected addBufferToRender() called while waiting for a texture.");
        ...
        public void onTextureFrameAvailable(int oesTextureId, float[] transformMatrix, long timestampNs) {
            synchronized (this.newFrameLock) {
                if (this.renderedBuffer != null) {
                    Logging.m173e(MediaCodecVideoDecoder.TAG, "Unexpected onTextureFrameAvailable() called while already holding a texture.");
    ...
    public static void disableVp8HwCodec() {
        Logging.m176w(TAG, "VP8 decoding is disabled by application.");
        hwDecoderDisabledTypes.add(VP8_MIME_TYPE);
    }

    public static void disableVp9HwCodec() {
        Logging.m176w(TAG, "VP9 decoding is disabled by application.");
        hwDecoderDisabledTypes.add(VP9_MIME_TYPE);
    }

    public static void disableH264HwCodec() {
        Logging.m176w(TAG, "H.264 decoding is disabled by application.");
    ...
    public static void printStackTrace() {
        if (runningInstance != null && runningInstance.mediaCodecThread != null) {
            ...
            if (mediaCodecStackTraces.length > 0) {
                Logging.m172d(TAG, "MediaCodecVideoDecoder stacks trace:");
    ...
    private static DecoderProperties findDecoder(String mime, String[] supportedCodecPrefixes) {
        ...
        Logging.m172d(TAG, "Trying to find HW decoder for mime " + mime);
        for (int i = 0; i < MediaCodecList.getCodecCount(); i++) {
            MediaCodecInfo info = null;
            try {
                info = MediaCodecList.getCodecInfoAt(i);
            } catch (IllegalArgumentException e) {
                Logging.m174e(TAG, "Cannot retrieve decoder codec info", e);
            }
            if (!(info == null || info.isEncoder())) {
                ...
                if (name != null) {
                    Logging.m172d(TAG, "Found candidate decoder " + name);
                    ...
                    if (supportedCodec) {
                        try {
                            ...
                            for (Integer intValue : supportedColorList) {
                                ...
                                for (int codecColorFormat : capabilities.colorFormats) {
                                    if (codecColorFormat == supportedColorFormat) {
                                        Logging.m172d(TAG, "Found target decoder " + name + ". Color: 0x" + Integer.toHexString(codecColorFormat));
                            ...
                            Logging.m174e(TAG, "Cannot retrieve decoder capabilities", e2);
                        }
                    } else {
                        continue;
                    }
        ...
        Logging.m172d(TAG, "No HW decoder found for mime " + mime);
        return null;
    }
    ...
    private boolean initDecode(VideoCodecType type, int width, int height, SurfaceTextureHelper surfaceTextureHelper) {
        ...
        Logging.m172d(TAG, "Java initDecode: " + type + " : " + width + " x " + height + ". Color: 0x" + Integer.toHexString(properties.colorFormat) + ". Use Surface: " + this.useSurface);
        ...
        try {
            ...
            if (this.mediaCodec == null) {
                Logging.m173e(TAG, "Can not create media decoder");
                return false;
            }
            ...
            Logging.m172d(TAG, "Input buffers: " + this.inputBuffers.length + ". Output buffers: " + this.outputBuffers.length);
            return true;
        } catch (IllegalStateException e) {
            Logging.m174e(TAG, "initDecode failed", e);
            return false;
        }
    }
    ...
    private void reset(int width, int height) {
        ...
        Logging.m172d(TAG, "Java reset: " + width + " x " + height);
    ...
    private void release() {
        Logging.m172d(TAG, "Java releaseDecoder. Total number of dropped frames: " + this.droppedFrames);
        checkOnMediaCodecThread();
        final CountDownLatch releaseDone = new CountDownLatch(1);
        new Thread(new Runnable() {
            public void run() {
                try {
                    Logging.m172d(MediaCodecVideoDecoder.TAG, "Java releaseDecoder on release thread");
                    MediaCodecVideoDecoder.this.mediaCodec.stop();
                    MediaCodecVideoDecoder.this.mediaCodec.release();
                    Logging.m172d(MediaCodecVideoDecoder.TAG, "Java releaseDecoder on release thread done");
                } catch (Exception e) {
                    Logging.m174e(MediaCodecVideoDecoder.TAG, "Media decoder release failed", e);
                }
                releaseDone.countDown();
            }
        }).start();
        if (!ThreadUtils.awaitUninterruptibly(releaseDone, 5000)) {
            Logging.m173e(TAG, "Media decoder release timeout");
            codecErrors++;
        ...
        Logging.m172d(TAG, "Java releaseDecoder done");
    }
    ...
    private boolean queueInputBuffer(int inputBufferIndex, int size, long presentationTimeStamUs, long timeStampMs, long ntpTimeStamp) {
        ...
        try {
            ...
            Logging.m174e(TAG, "decode failed", e);
            return false;
        }
    }
        ...
        while (true) {
            ...
            switch (result) {
                case ProfilePictureView.NORMAL /*-3*/:
                    this.outputBuffers = this.mediaCodec.getOutputBuffers();
                    Logging.m172d(TAG, "Decoder output buffers changed: " + this.outputBuffers.length);
                ...
                case -2:
                    ...
                    Logging.m172d(TAG, "Decoder format changed: " + format.toString());
                    if (format.containsKey(FORMAT_KEY_CROP_LEFT)) {
                        if (format.containsKey(FORMAT_KEY_CROP_RIGHT)) {
                            if (format.containsKey(FORMAT_KEY_CROP_BOTTOM)) {
                                    ...
                                    if (this.hasDecodedFirstFrame || (newWidth == this.width && newHeight == this.height)) {
                                        ...
                                        if (!this.useSurface) {
                                            if (format.containsKey("color-format")) {
                                                ...
                                                Logging.m172d(TAG, "Color: 0x" + Integer.toHexString(this.colorFormat));
                                        ...
                                        Logging.m172d(TAG, "Frame stride and slice height: " + this.stride + " x " + this.sliceHeight);
                    ...
                    newWidth = format.getInteger(SettingsJsonConstants.ICON_WIDTH_KEY);
                    newHeight = format.getInteger(SettingsJsonConstants.ICON_HEIGHT_KEY);
                    if (this.hasDecodedFirstFrame) {
                        break;
                    }
                    this.width = newWidth;
                ...
                default:
                    ...
                    if (decodeTimeMs > 200) {
                        Logging.m173e(TAG, "Very high decode time: " + decodeTimeMs + "ms" + ". Q size: " + this.decodeStartTimeMs.size() + ". Might be caused by resuming H264 decoding after a pause.");
                        decodeTimeMs = 200;
                    }
        ...
        if (this.useSurface) {
            ...
            if (renderedBuffer != null) {
                ...
                if (dequeueTimeoutMs > 0) {
                    Logging.m176w(TAG, "Draining decoder. Dropping frame with TS: " + droppedFrame.presentationTimeStampMs + ". Total number of dropped frames: " + this.droppedFrames);
                } else {
                    Logging.m176w(TAG, "Too many output buffers " + this.dequeuedSurfaceOutputBuffers.size() + ". Dropping frame with TS: " + droppedFrame.presentationTimeStampMs + ". Total number of dropped frames: " + this.droppedFrames);
                }
