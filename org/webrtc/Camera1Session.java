...
public class Camera1Session implements CameraSession {
    ...
    class C15991 implements ErrorCallback {
        C15991() {
        }
        ...
        public void onError(int error, Camera camera) {
            ...
            Logging.m173e(Camera1Session.TAG, errorMessage);
    ...
    class C16002 implements OnTextureFrameAvailableListener {
        C16002() {
        }
        ...
        public void onTextureFrameAvailable(int oesTextureId, float[] transformMatrix, long timestampNs) {
            ...
            if (Camera1Session.this.state != SessionState.RUNNING) {
                Logging.m172d(Camera1Session.TAG, "Texture frame captured but camera is no longer running.");
    ...
    class C16013 implements PreviewCallback {
        C16013() {
        }
        ...
        public void onPreviewFrame(byte[] data, Camera callbackCamera) {
            ...
            if (callbackCamera != Camera1Session.this.camera) {
                Logging.m173e(Camera1Session.TAG, "Callback from a different camera. This should never happen.");
            } else if (Camera1Session.this.state != SessionState.RUNNING) {
                Logging.m172d(Camera1Session.TAG, "Bytebuffer frame captured but camera is no longer running.");
            } else {
    ...
    public static void create(CreateSessionCallback callback, Events events, boolean captureToTexture, Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, int cameraId, int width, int height, int framerate) {
        ...
        Logging.m172d(TAG, "Open camera " + cameraId);
    ...
    private Camera1Session(Events events, boolean captureToTexture, Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, int cameraId, int width, int height, int framerate, Camera camera, CameraInfo info, CaptureFormat captureFormat, long constructionTimeNs) {
        Logging.m172d(TAG, "Create new camera1 session on camera " + cameraId);
    ...
    public void stop() {
        Logging.m172d(TAG, "Stop camera1 session on camera " + this.cameraId);
    ...
    private void startCapturing() {
        Logging.m172d(TAG, "Start capturing");
        checkIsOnCameraThread();
        this.state = SessionState.RUNNING;
        this.camera.setErrorCallback(new C15991());
    ...
    private void listenForTextureFrames() {
        this.surfaceTextureHelper.startListening(new C16002());
    }

    private void listenForBytebufferFrames() {
        this.camera.setPreviewCallbackWithBuffer(new C16013());
    }
