...
public class Camera2Session implements CameraSession {
    ...
    private class CameraCaptureCallback extends CaptureCallback {
        ...
        public void onCaptureFailed(CameraCaptureSession session, CaptureRequest request, CaptureFailure failure) {
            Logging.m172d(Camera2Session.TAG, "Capture failed: " + failure);
        }
    }
    ...
    private class CameraStateCallback extends StateCallback {
        ...
        public void onOpened(CameraDevice camera) {
            Camera2Session.this.checkIsOnCameraThread();
            Logging.m172d(Camera2Session.TAG, "Camera opened.");
        ...
        public void onClosed(CameraDevice camera) {
            Camera2Session.this.checkIsOnCameraThread();
            Logging.m172d(Camera2Session.TAG, "Camera device closed.");
    ...
    private class CaptureSessionCallback extends CameraCaptureSession.StateCallback {
        ...
        class C16031 implements OnTextureFrameAvailableListener {
            C16031() {
            }
            ...
            public void onTextureFrameAvailable(int oesTextureId, float[] transformMatrix, long timestampNs) {
                ...
                if (Camera2Session.this.state != SessionState.RUNNING) {
                    Logging.m172d(Camera2Session.TAG, "Texture frame captured but camera is no longer running.");
        ...
        public void onConfigured(CameraCaptureSession session) {
            Camera2Session.this.checkIsOnCameraThread();
            Logging.m172d(Camera2Session.TAG, "Camera capture session configured.");
            ...
            try {
                ...
                Camera2Session.this.surfaceTextureHelper.startListening(new C16031());
                Logging.m172d(Camera2Session.TAG, "Camera device successfully started.");
        ...
        private void chooseStabilizationMode(Builder captureRequestBuilder) {
            ...
            if (availableOpticalStabilization != null) {
                for (int mode : availableOpticalStabilization) {
                    if (mode == 1) {
                        ...
                        Logging.m172d(Camera2Session.TAG, "Using optical stabilization.");
                        return;
                    }
                }
            }
            for (int mode2 : (int[]) Camera2Session.this.cameraCharacteristics.get(CameraCharacteristics.CONTROL_AVAILABLE_VIDEO_STABILIZATION_MODES)) {
                if (mode2 == 1) {
                    ...
                    Logging.m172d(Camera2Session.TAG, "Using video stabilization.");
                    return;
                }
            }
            Logging.m172d(Camera2Session.TAG, "Stabilization not available.");
        }
        ...
        private void chooseFocusMode(Builder captureRequestBuilder) {
            for (int mode : (int[]) Camera2Session.this.cameraCharacteristics.get(CameraCharacteristics.CONTROL_AF_AVAILABLE_MODES)) {
                if (mode == 3) {
                    ...
                    Logging.m172d(Camera2Session.TAG, "Using continuous video auto-focus.");
                    return;
                }
            }
            Logging.m172d(Camera2Session.TAG, "Auto-focus is not available.");
        }
    }
    ...
    private Camera2Session(CreateSessionCallback callback, Events events, Context applicationContext, CameraManager cameraManager, SurfaceTextureHelper surfaceTextureHelper, String cameraId, int width, int height, int framerate) {
        Logging.m172d(TAG, "Create new camera2 session on camera " + cameraId);
    ...
    private void findCaptureFormat() {
        ...
        Logging.m172d(TAG, "Available preview sizes: " + sizes);
        Logging.m172d(TAG, "Available fps ranges: " + framerateRanges);
        ...
        Logging.m172d(TAG, "Using capture format: " + this.captureFormat);
    }

    private void openCamera() {
        checkIsOnCameraThread();
        Logging.m172d(TAG, "Opening camera " + this.cameraId);
    ...
    public void stop() {
        Logging.m172d(TAG, "Stop camera2 session on camera " + this.cameraId);
        checkIsOnCameraThread();
    ...
    private void stopInternal() {
        Logging.m172d(TAG, "Stop internal");
        checkIsOnCameraThread();
        ...
        Logging.m172d(TAG, "Stop done");
    }

    private void reportError(String error) {
        checkIsOnCameraThread();
        Logging.m173e(TAG, "Error: " + error);
