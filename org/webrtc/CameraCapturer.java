...
public abstract class CameraCapturer implements CameraVideoCapturer {
    ...
    private final Events cameraSessionEventsHandler = new C16052();
    private CameraStatistics cameraStatistics;
    private Handler cameraThreadHandler;
    private CapturerObserver capturerObserver;
    private final CreateSessionCallback createSessionCallback = new C16041();
    ...
    private final Runnable openCameraTimeoutRunnable = new C16063();
    ...
    class C16041 implements CreateSessionCallback {
        C16041() {
        }

        public void onDone(CameraSession session) {
            CameraCapturer.this.checkIsOnCameraThread();
            Logging.m172d(CameraCapturer.TAG, "Create session done");
        ...
        public void onFailure(String error) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                if (CameraCapturer.this.openAttemptsRemaining <= 0) {
                    Logging.m176w(CameraCapturer.TAG, "Opening camera failed, passing: " + error);
                    CameraCapturer.this.sessionOpening = false;
                    CameraCapturer.this.stateLock.notifyAll();
                    ...
                    Logging.m176w(CameraCapturer.TAG, "Opening camera failed, retry: " + error);
    ...
    class C16052 implements Events {
        C16052() {
        }
        ...
        public void onCameraOpening() {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (CameraCapturer.this.currentSession != null) {
                    Logging.m176w(CameraCapturer.TAG, "onCameraOpening while session was open.");
                    return;
                }
        ...
        public void onCameraError(CameraSession session, String error) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (session != CameraCapturer.this.currentSession) {
                    Logging.m176w(CameraCapturer.TAG, "onCameraError from another session: " + error);
                    return;
                }
                CameraCapturer.this.eventsHandler.onCameraError(error);
        ...
        public void onCameraDisconnected(CameraSession session) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (session != CameraCapturer.this.currentSession) {
                    Logging.m176w(CameraCapturer.TAG, "onCameraDisconnected from another session.");
                    return;
                }
        ...
        public void onCameraClosed(CameraSession session) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                Logging.m172d(CameraCapturer.TAG, "onCameraClosed from another session.");
            }
        }
        ...
        public void onByteBufferFrameCaptured(CameraSession session, byte[] data, int width, int height, int rotation, long timestamp) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (session != CameraCapturer.this.currentSession) {
                    Logging.m176w(CameraCapturer.TAG, "onByteBufferFrameCaptured from another session.");
                    return;
                }
        ...
        public void onTextureFrameCaptured(CameraSession session, int width, int height, int oesTextureId, float[] transformMatrix, int rotation, long timestamp) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (session != CameraCapturer.this.currentSession) {
                    Logging.m176w(CameraCapturer.TAG, "onTextureFrameCaptured from another session.");
    ...
    class C16074 implements CameraEventsHandler {
        C16074() {
        }
    ...
    class C16085 implements Runnable {
        C16085() {
        }
    ...
    public void startCapture(int width, int height, int framerate) {
        Logging.m172d(TAG, "startCapture: " + width + "x" + height + "@" + framerate);
        synchronized (this.stateLock) {
            if (this.sessionOpening || this.currentSession != null) {
                Logging.m176w(TAG, "Session already open");
                return;
            }
            this.width = width;
            this.height = height;
    ...
    private void createSessionInternal(int delayMs) {
        ...
        this.cameraThreadHandler.postDelayed(new C16085(), (long) delayMs);
    }

    public void stopCapture() {
        Logging.m172d(TAG, "Stop capture");
        synchronized (this.stateLock) {
            while (this.sessionOpening) {
                Logging.m172d(TAG, "Stop capture: Waiting for session to open");
                ThreadUtils.waitUninterruptibly(this.stateLock);
            }
            if (this.currentSession != null) {
                Logging.m172d(TAG, "Stop capture: Nulling session");
                ...
                Logging.m172d(TAG, "Stop capture: No session open");
            }
        }
        Logging.m172d(TAG, "Stop capture done");
    }

    public void changeCaptureFormat(int width, int height, int framerate) {
        Logging.m172d(TAG, "changeCaptureFormat: " + width + "x" + height + "@" + framerate);
        synchronized (this.stateLock) {
            stopCapture();
    ...
    public void switchCamera(final CameraSwitchHandler switchEventsHandler) {
        Logging.m172d(TAG, "switchCamera");
        this.cameraThreadHandler.post(new Runnable() {
            public void run() {
    ...
    public void printStackTrace() {
        ...
        if (cameraThread != null) {
            ...
            if (cameraStackTrace.length > 0) {
                Logging.m172d(TAG, "CameraCapturer stack trace:");
                for (StackTraceElement traceElem : cameraStackTrace) {
                    Logging.m172d(TAG, traceElem.toString());
                }
            }
        }
    }
    ...
    private void switchCameraInternal(org.webrtc.CameraVideoCapturer.CameraSwitchHandler r7) {
        ...
        org.webrtc.Logging.m172d(r3, r4);
    ...
    L_0x0022:
        ...
        org.webrtc.Logging.m172d(r3, r5);	 Catch:{ all -> 0x0032 }
    ...
    private void checkIsOnCameraThread() {
        if (Thread.currentThread() != this.cameraThreadHandler.getLooper().getThread()) {
            Logging.m173e(TAG, "Check is on camera thread failed.");
