...
public final class BlockingObservable<T> {
    ...
    private final Observable<? extends T> f243o;

    class C19582 implements Iterable<T> {
        C19582() {
        }
    ...
    class C19648 implements Action1<Throwable> {
        C19648() {
        }
    ...
    public void forEach(final Action1<? super T> onNext) {
        ...
        BlockingUtils.awaitForComplete(latch, this.f243o.subscribe(new Subscriber<T>() {
    ...
    public Iterator<T> getIterator() {
        return BlockingOperatorToIterator.toIterator(this.f243o);
    }

    public T first() {
        return blockForSingle(this.f243o.first());
    }

    public T first(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f243o.first(predicate));
    }

    public T firstOrDefault(T defaultValue) {
        return blockForSingle(this.f243o.map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T firstOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f243o.filter(predicate).map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T last() {
        return blockForSingle(this.f243o.last());
    }

    public T last(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f243o.last(predicate));
    }

    public T lastOrDefault(T defaultValue) {
        return blockForSingle(this.f243o.map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public T lastOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f243o.filter(predicate).map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public Iterable<T> mostRecent(T initialValue) {
        return BlockingOperatorMostRecent.mostRecent(this.f243o, initialValue);
    }

    public Iterable<T> next() {
        return BlockingOperatorNext.next(this.f243o);
    }

    public Iterable<T> latest() {
        return BlockingOperatorLatest.latest(this.f243o);
    }

    public T single() {
        return blockForSingle(this.f243o.single());
    }

    public T single(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f243o.single(predicate));
    }

    public T singleOrDefault(T defaultValue) {
        return blockForSingle(this.f243o.map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public T singleOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f243o.filter(predicate).map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public Future<T> toFuture() {
        return BlockingOperatorToFuture.toFuture(this.f243o);
    }
    ...
    public void subscribe() {
        ...
        BlockingUtils.awaitForComplete(cdl, this.f243o.subscribe(new Subscriber<T>() {
    ...
    public void subscribe(Observer<? super T> observer) {
        ...
        Subscription s = this.f243o.subscribe(new Subscriber<T>() {
            public void onNext(T t) {
    ...
    public void subscribe(Action1<? super T> onNext) {
        subscribe(onNext, new C19648(), Actions.empty());
    }
