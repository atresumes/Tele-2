...
public final class BufferUntilSubscriber<T> extends Subject<T, T> {
    static final Observer EMPTY_OBSERVER = new C17521();
    private boolean forward = false;
    final State<T> state;

    static class C17521 implements Observer {
        C17521() {
        }
    ...
    static final class OnSubscribeAction<T> implements OnSubscribe<T> {
        ...
        class C17531 implements Action0 {
            C17531() {
            }
        ...
        public void call(Subscriber<? super T> s) {
            if (this.state.casObserverRef(null, s)) {
                s.add(Subscriptions.create(new C17531()));
                boolean win = false;
